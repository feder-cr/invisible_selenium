...
def seed_recaptcha_cookies(context: Any, profile: Any,
                           locale: Optional[str] = None) -> None:
    """Inject the persona's deterministic cookies into an engine context.

    ⛔ ONE FUNCTION WHERE invisible_playwright HAS TWO. There the context is a
    Playwright object with a sync and an async `add_cookies`; here it is the
    engine's `Context`, whose `set_cookies` speaks Juggler's cookie shape - the
    same shape `build_cookies` already produces, since Playwright passed it
    through untouched.
    """
